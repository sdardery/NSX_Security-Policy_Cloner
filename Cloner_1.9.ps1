...
function Get-FullGroupChain {
    ...
    $Cache[$GroupPath] = @([PSCustomObject]@{ path = $GroupPath; body = $json })
        ...
        foreach ($expr in $raw.expression) {
            if ($expr.paths) {
                foreach ($p in $expr.paths) {
                    if ($p -and $p.StartsWith("/infra/domains/")) { $children += $p }
                }
            }
        }
    }
...
function Get-FullServiceChain {
    ...
    $json = $raw | ConvertTo-Json -Depth 30
    $Cache[$Path] = @([PSCustomObject]@{ path = $Path; body = $json })

    $children = @()
    if ($raw.service_entries) {
        foreach ($e in $raw.service_entries) {
            $nested = ([string]$e.nested_service_path).Trim()
            if ($nested -and $nested.StartsWith("/infra/services/")) { $children += $nested }
        }
    }
...
foreach ($policy in $policies)
{
    ...
    foreach ($rule in $policyJson.rules)
    {
        ...
        if ($rule.scope) {
            foreach ($s in $rule.scope) { if ($s -ne "ANY") { $allGroups += $s } }
        }
        ...
        foreach ($grp in $allGroups)
        {
            ...
            $unique = @{}
            foreach ($item in $chainG) { if (-not $unique.ContainsKey($item.path)) { $unique[$item.path] = $item.body } }
        ...
        foreach ($service in $rule.services)
        {
            ...
            $unique = @{}
            foreach ($item in $chain) { if (-not $unique.ContainsKey($item.path)) { $unique[$item.path] = $item.body } }
        ...
        try {
            ...
            if ($resp) {
                Write-Output "`e[32m   ✅ Rule PATCH API succeeded: $($rule.path)`e[0m"
            }
            else {
                Write-Output "`e[33m   ⚠️ PATCH returned empty response but no error: $($rule.path)`e[0m"
            }
        }
        catch {
            Write-Output "`e[31m   ❌ Rule PATCH failed (actual API error): $($rule.path)`e[0m"
        }
    }
