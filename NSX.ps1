...
# Version: NSX Final Clean Script 1.6
# =====================================================================================
    ...
    if ($raw.service_entries) {
        foreach ($e in $raw.service_entries) {
            ...
            if (
                $nested -ne "" -and
                $nested -ne "/" -and
                $nested.StartsWith("/infra/services/")
            ) {
                $children += $nested
            }
        }
...
foreach ($policy in $policies)
{
    ...
    foreach ($rule in $policyJson.rules)
    {
        ...
        # =====================================================================
        # GROUPS (source + destination + scope)
        # =====================================================================
        $allGroups = @()
        ...
        foreach ($service in $rule.services)
        {
            ...
            # Check if parent is system service
            try {
